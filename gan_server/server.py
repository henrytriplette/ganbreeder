...
parser.add_argument('--seed', default=2357, type=int,
                    help='The filename of image to be completed.')
parser.add_argument('--truncation', default=0.55, type=float,
                    help='Place mask on top, bottom or middle.')
parser.add_argument('--port', default=5000, type=float,
                    help='Place mask on top, bottom or middle.')

args = parser.parse_args()

# module_path = 'https://tfhub.dev/deepmind/biggan-512/2' # Lower quality, more trippy
module_path = 'https://tfhub.dev/deepmind/biggan-deep-512/1' # Less tryppy
rand_seed = args.seed
...
# Mine edits for CUBLAS_STATUS_ALLOC_FAILED
...
def sample(vectors, labels, batch_size=5):
    num = vectors.shape[0]
    ims = []
...
def create_variations(num, vector, label):
    ...
    for i in range(num):
        ...
        if random.random() < 0.1:
            new_labels[i][random.randint(0, label.shape[0]-1)] += random.random() * 0.6
...
def get_random():
    try:
        ...
        print('Random', num)
        t = time.time()
        imgs, vectors, labels = create_random_images(num, max_classes=4)
...
def children():
    try:
        ...
        new_vectors, new_labels = create_variations(4, vector, label)
...
def mix_images():
    try:
        ...
        new_vectors, new_labels = interpolate(4, vector1, vector2, label1, label2)
        new_ims = sample(new_vectors, new_labels)
        return jsonify([
            [ encode_img(arr) for arr in new_ims ],
            new_vectors.tolist(),
